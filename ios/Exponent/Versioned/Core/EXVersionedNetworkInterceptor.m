...
@interface EXVersionedNetworkInterceptor () <ExpoRequestCdpLoggerDelegate>
...
- (instancetype)initWithRCTInspectorPackagerConnection:(RCTInspectorPackagerConnection *)inspectorPackgerConnection
{
  if (self = [super init]) {
    ...
    Class requestInterceptorClass = [ExpoRequestInterceptorProtocol class];
    ...
    NSURLSessionConfiguration *urlSessionConfiguration = [[SDWebImageDownloader.sharedDownloader valueForKey:@"_session"] configuration];
    NSMutableArray<Class> *protocolClasses = [urlSessionConfiguration.protocolClasses mutableCopy];
    if (![protocolClasses containsObject:requestInterceptorClass]) {
      [protocolClasses insertObject:requestInterceptorClass atIndex:0];
      urlSessionConfiguration.protocolClasses = protocolClasses;
      NSURLSession *session = [NSURLSession sessionWithConfiguration:urlSessionConfiguration
                                                            delegate:SDWebImageDownloader.sharedDownloader
                                                       delegateQueue:nil];
      [SDWebImageDownloader.sharedDownloader setValue:session forKey:@"_session"];
    }
  }
  return self;
}
...
#pragma mark - ExpoRequestCdpLoggerDelegate implementations
